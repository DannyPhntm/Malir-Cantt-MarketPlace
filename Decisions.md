...
## Pre-beta security & UX audit — 2026-07-02

Multi-perspective read-only audit (security / backend / frontend) before private-beta onboarding, then smallest-safe fixes on branch `fix/beta-security-audit`. No feature removed; no production data touched. Key decisions:

- **Public listing feed is status-enforced server-side.** `GET /listings` previously trusted the `status` query param and (with none) returned *every* status incl. pending/hidden/rejected with seller phone numbers — anyone could read unmoderated content. Now non-admins are forced to `status='approved'` (or `sold` when explicitly requested); only admins (fresh role via `optionalAuth` DB lookup) may filter moderation queues or use `?featuredRequested=`. This is the real enforcement of the "private listings are 404" rule that previously lived only on the detail route.
- **Email verification is now mandatory to log in.** Login returned a full JWT to accounts that never verified their email. Now `login` 403s unverified accounts with `{ code: 'EMAIL_UNVERIFIED', unverified, email }`; the sign-in screen routes straight to the verify/resend flow. Verification is no longer bypassable.
- **Saved-listings can't be used as a side door.** `POST /saved` now 404s non-public listings (mirrors the detail route) and `GET /saved` filters out listings that went private after saving (unless the saver owns them).
- **Approved-listing edits go back to `pending` (bait-and-switch guard).** A material edit (title/description/price/subcategory/details/images) to an otherwise-live listing re-enters moderation. Pure lifecycle changes (mark sold/hide/restore) don't. "Kept" image URLs on edit must belong to the listing's existing image set — arbitrary external URLs can no longer be injected into a live listing.
- **Business verification / CNIC documents are `authenticated`-type Cloudinary uploads.** Previously world-readable by URL; now not publicly fetchable — admins view them via short-lived signed URLs (`signedDocUrl`). Re-applications destroy the replaced doc assets (`destroyDocAsset`); the account row + `accountType` write is atomic.
- **Business revocation cascades.** Admin flipping approved→rejected/pending now hides the owner's shop and live business listings (reversible; featured flags cleared). Previously a revoked business kept its public presence indefinitely.
- **Re-activation re-checks the beta active-listing limit** (hiding listings could stockpile extras past the cap), and expired featured windows no longer count in stats or block re-requests.
- **Defense-in-depth:** all `/:id` route params validated (`idParamSchema` — NaN/float ids now 422 not 500); body-parser errors → 400/413 not 500; price capped at 1e9 (Int overflow → 500); `details` map capped (≤40 keys, key length ≤60); `updateUser` uses an explicit field allow-list (not `data: req.body`); avatar + shop images run through the Cloudinary pipeline instead of raw base64 in Postgres; login timing-equalised against user enumeration; admin user search case-insensitive; `optionalAuth` re-checks role/blocked from the DB (stale token can't retain admin/owner access); `createLimiter`/`uploadLimiter` (30/hr) on listing/shop/business create+edit; CORS + Resend fail loudly at boot in production if `CLIENT_ORIGIN`/`RESEND_API_KEY` unset (no code fallback that logs verification codes in prod).
- **Frontend:** `?redirect=` restricted to same-origin in-app paths (open-redirect guard); global 401-mid-session handler clears the dead token + drops to logged-out state; guest favourites no longer destroyed on a failed login-merge; seller profiles keyed by owner id (names aren't unique — a name-keyed page merged different sellers and showed the wrong phone); Terms/Community-Guidelines consent links point at `/terms` and `/safety` (were `/`); public-stats cache gets a 60s TTL; add-listing draft cleared on logout (shared-device hygiene).

Checks run: eslint (clean), `prisma validate`, 4 server test scripts (limits/blocking/business-verification/image-validation — all pass), frontend `vite build` (pass), and live API smoke tests (status enforcement, private-listing 404, NaN→422, malformed-JSON→400, auth gates 401, security headers, CORS rejection, no private-field leaks in public responses — all confirmed). Not merged/deployed pending review. Live full-browser responsive pass still pending (deferred by intermittent DB/network during the session).
